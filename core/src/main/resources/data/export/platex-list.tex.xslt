...
%short-description: pLaTeX - translations at end
%description
% Generate a platex input file. Translations are put in
% a list after the main text.
%end description
...
\bigskip
    ...
    <xsl:template match="div" mode="annotationlist-1">
      <xsl:text>\para{</xsl:text><xsl:number count="div" /><xsl:text>}
